...
/**
 * A container holding most data needed to be carried between components of the
 * control interface. <br>
 * It keeps track of current time and illumination option, as well as a list of
 * the corresponding switching times.<br>
 * It further keeps track of the display state and log file paths. <br>
 * Finally, it also contains a communicator object, for interaction with an
 * Arduino
 * 
 * @author Erik Hedlund
 *
...
//		setGotoPos(0);
	...
	/**
	 * @return how often the automatic RTC precision check should be performed, in
	 *         seconds.
	 */
	...
	/**
	 * @param timeChInt : How often the automatic RTC precision check should be
	 *                  performed, in seconds. Defaults to one hour (3600 s).
	 */
	...
	/**
	 * @param aDrift : Allowed minor drift before a set event will occur. Defaults
	 *               to 29 seconds.
	 */
	...
	public void loadPrefs(Properties loadedPrefs, Properties defaults) {
		prefs = loadedPrefs;
		loBr = Integer.parseInt(prefs.getProperty("loBr", defaults.getProperty("loBr", "7"))); // prefs.getProperty("loBr",
																								// "31")); //
...
//		gotoPos = currPos;
		// timeout = Long.parseLong(prefs.getProperty("timeout",
		// defaults.getProperty("timeout", "0")));
		...
		timeAutoSet = Boolean
				.parseBoolean(prefs.getProperty("timeAutoSet", defaults.getProperty("timeAutoSet", "false")));
//		System.out.print("Hardcoded timeChInt: ");
//		System.out.println(timeChInt);
		timeChInt = Integer.parseInt(prefs.getProperty("timeChInt", defaults.getProperty("timeChInt", "3600")));
